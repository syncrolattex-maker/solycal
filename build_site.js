...
<head>
  ...
  <style>
    ...
    /* PRELOADER INTRO INDUSTRIAL SOLYCAL (ANIMACIÓN DE CARGA) */
    #solycal-loader {
      position: fixed;
      inset: 0;
      z-index: 99999;
    ...
    .loader-ring {
      position: absolute;
      border-radius: 50%;
      border: 1px dashed rgba(241, 181, 65, 0.25);
...
function getHeader(activeSlug) {
  ...
  return `
  <!-- PANTALLA DE CARGA INICIAL (PRELOADER CON ANIMACIÓN DE LOGO SOLYCAL) -->
  <div id="solycal-loader" aria-hidden="false">
    <!-- Anillos concéntricos de precisión industrial -->
    <div class="loader-ring loader-ring-outer"></div>
    <div class="loader-ring loader-ring-inner"></div>

    <div class="relative z-10 flex flex-col items-center">
      <!-- Icono del Logo con halo de brillo radiante -->
      <div id="loader-logo-wrap" class="relative flex items-center justify-center mb-6">
        <div class="absolute inset-0 rounded-full bg-brand-yellow/15 blur-2xl transform scale-150"></div>
        <img id="loader-logo-icon" src="assets/logo-icon.png" alt="Solycal" class="relative z-10 w-16 sm:w-20 h-auto object-contain drop-shadow-[0_0_20px_rgba(241,181,65,0.4)]">
      </div>

      <!-- Nombre de la marca que se expande -->
      <div class="overflow-hidden flex items-center justify-center mb-4">
        <span id="loader-logo-text" class="font-display font-bold text-2xl sm:text-3xl tracking-[0.3em] text-white uppercase inline-block">
          SOLYCAL
        </span>
      </div>

      <!-- Subtítulo técnico y barra de progreso industrial -->
      <div class="flex flex-col items-center gap-2">
        <div class="w-44 h-[2px] bg-white/10 rounded-full overflow-hidden relative">
          <div id="loader-progress-bar" class="absolute left-0 top-0 bottom-0 bg-brand-yellow w-0 rounded-full shadow-[0_0_10px_#F1B541]"></div>
        </div>
        <div class="flex items-center justify-between w-44 font-mono text-[10px] text-neutral-400 uppercase tracking-widest pt-1">
          <span id="loader-status-text">PRECISIÓN INDUSTRIAL</span>
          <span id="loader-percent" class="text-brand-yellow font-semibold">0%</span>
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    window.addEventListener('DOMContentLoaded', () => {
      ...
      if (typeof gsap !== 'undefined') {
        ...
        // =========================================================================
        // PRELOADER INDUSTRIAL CON ANIMACIÓN DE LOGO SOLYCAL (LOADING SCREEN)
        // =========================================================================
        const solycalLoader = document.getElementById('solycal-loader');
        const loaderIcon = document.getElementById('loader-logo-icon');
        const loaderText = document.getElementById('loader-logo-text');
        ...
        // Función para ejecutar la animación del Hero una vez se levante el loader
        const playHeroEntrance = () => {
          if (document.querySelector('.hero-sub')) {
            const heroTl = gsap.timeline({ defaults: { ease: "power3.out" } });
            heroTl
              .from('.hero-sub', { opacity: 0, y: -20, duration: 0.8 })
              .from('.hero-tag', { opacity: 0, x: -25, duration: 0.7 }, "-=0.4")
              .from('.hero-title', { opacity: 0, y: 35, duration: 1, ease: "power4.out" }, "-=0.5")
              .from('.hero-desc', { opacity: 0, y: 25, duration: 0.9 }, "-=0.6")
              .from('.hero-actions a', { opacity: 0, y: 20, stagger: 0.15, duration: 0.7 }, "-=0.5")
              .from('#bg-video', { opacity: 0, scale: 1.15, duration: 1.5, ease: "power2.out" }, 0);
          }
        };

        // Verificamos si ya se vio el preloader en esta sesión para no ralentizar navegación interna repetida
        ...
        if (solycalLoader && !hasLoadedBefore) {
          ...
          // Estado inicial de los elementos del loader
          gsap.set(loaderIcon, { opacity: 0, scale: 0.3, rotation: -20, filter: "brightness(0.5)" });
          gsap.set(loaderText, { opacity: 0, letterSpacing: "0.8em", y: 20 });
          gsap.set(loaderBar, { width: "0%" });

          const loadTl = gsap.timeline({
            onComplete: () => {
              // Desaparece el preloader de forma suave
              gsap.to(solycalLoader, {
                opacity: 0,
                scale: 1.04,
                duration: 0.7,
                ease: "power3.inOut",
          ...
          // 1. Aparición impactante del icono del logo con destello y rotación elástica
          loadTl
            .to(loaderIcon, {
              opacity: 1,
              scale: 1,
            ...
            // 2. Revelado del texto SOLYCAL con expansión de kerning
            .to(loaderText, {
              opacity: 1,
              letterSpacing: "0.3em",
              y: 0,
            ...
            // 3. Simulación de carga fluida industrial 0% -> 100%
            .to(loaderBar, {
              width: "100%",
              duration: 1.2,
              ease: "power2.inOut",
              onUpdate: function() {
                ...
                if (loaderStatus) {
                  if (progress > 75) loaderStatus.textContent = 'LISTO';
                  else if (progress > 45) loaderStatus.textContent = 'CALDERERÍA & CORTE HD';
                  else loaderStatus.textContent = 'PRECISIÓN INDUSTRIAL';
                }
              }
            }, "-=0.4")
            // 4. Pulso final de confirmación del logo
            .to(loaderIcon, {
              scale: 1.1,
              duration: 0.25,
              yoyo: true,
          ...
          // Si ya se ha visto en la sesión, ocultar directamente y disparar animación
        ...
        if (logoLink && logoIcon && logoText) {
          ...
          const logoTl = gsap.timeline({ delay: hasLoadedBefore ? 0.1 : 2.6 });
          logoTl
            .to(logoIcon, { 
              opacity: 1, 
              scale: 1, 
