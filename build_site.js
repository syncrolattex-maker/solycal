      ...
      if (typeof gsap !== 'undefined') {
        ...
        (function initWebGLPixelReveal() {
          ...
          const fragmentShader = [
            ...
            '  }',
            '',
            '  // Revelado pixelado: De cuadricula gruesa (36px) a resolucion nativa 1px',
            '  float maxPixelSize = 36.0;',
            '  float minPixelSize = 1.0;',
            '  float t = pow(uProgress, 2.0);',
            '  float pixelSize = mix(maxPixelSize, minPixelSize, t);',
            '',
            '  vec2 gridSize = max(vec2(1.0), uResolution / pixelSize);',
            '',
            '  // floor(vUv * gridSize) / gridSize',
            '  vec2 cellIndex = floor(vUv * gridSize);',
            '  vec2 pixelatedUv = (cellIndex + 0.5) / gridSize;',
            '',
            '  // Revelado aleatorio por celdas con ruido',
            '  float cellRand = hash21(cellIndex);',
            '  float revealProgress = clamp(uProgress / 0.72, 0.0, 1.0);',
            '  float cellAlpha = smoothstep(cellRand - 0.12, cellRand + 0.05, revealProgress);',
            '  if (cellAlpha <= 0.001) {',
            '    discard;',
            '  }',
            '',
            '  // Muestreo de textura: pixelado -> nitido progresivo',
            '  float blendSharp = smoothstep(0.70, 0.98, uProgress);',
            '  vec2 finalUv = mix(pixelatedUv, vUv, blendSharp);',
            '',
            ...
            '  }',
            '',
            '  float globalAlpha = smoothstep(0.0, 0.12, uProgress);',
            '  gl_FragColor = vec4(color.rgb, color.a * cellAlpha * globalAlpha);',
            '}'
          ].join('\\n');
          ...
          function triggerItemReveal(item) {
            ...
            // 5. ANIMACIÓN GSAP E INTERCAMBIO LIMPIO (0 a 1 en 0.9s)
