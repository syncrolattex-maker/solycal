  ...
  <script>
    ...
    function initApp() {
      ...
      if (typeof gsap !== 'undefined') {
        ...
        // =========================================================================
        // REVELADO PIXELADO CON WEBGL, THREE.JS Y GSAP SCROLLTRIGGER (ESTILO CODROPS)
        // Exclusivo para h1:not(.hero-title) y h2
        // =========================================================================
        (function initWebGLPixelReveal() {
          if (typeof THREE === 'undefined' || typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') return;

          // 1. ESCENA WEBGL SUPERPUESTA CON <canvas id="webgl">
          ...
          // Orthographic Camera en escala 1:1 con píxeles de pantalla (0, width, 0, -height)
          ...
          // 2. SELECCIÓN EXCLUSIVA DE ELEMENTOS: h1:not(.hero-title) y h2
          const targetElements = Array.from(document.querySelectorAll('h1:not(.hero-title), h2')).filter(el => {
            return !el.closest('#joby-nav-overlay') && !el.closest('#solycal-loader') && el.textContent.trim().length > 0;
          });

          if (targetElements.length === 0) return;

          // 3. SINCRONIZACIÓN DOM A TEXTURA (CANVAS 2D DE ALTA RESOLUCIÓN)
          function renderHeadingToCanvas(heading, dpr) {
            const headingRect = heading.getBoundingClientRect();
            const w = Math.ceil(headingRect.width);
            const h = Math.ceil(headingRect.height);
            ...
            while ((textNode = walker.nextNode())) {
              ...
              ctx.font = (computed.fontStyle || 'normal') + ' ' + (computed.fontWeight || '700') + ' ' + computed.fontSize + ' ' + computed.fontFamily;
              ...
              while (i < text.length) {
                ...
                const wordText = text.substring(i, j);

                if (wordRect.width > 0 && wordRect.height > 0) {
                  const x = wordRect.left - headingRect.left;
                  const y = (wordRect.top - headingRect.top) + wordRect.height * 0.5;
            ...
            return { canvas: c2d, width: w, height: h };
          }

          // 4. SHADER DE PIXELACIÓN (ShaderMaterial con uProgress 0.0 -> 1.0)
          const vertexShader = [
            'varying vec2 vUv;',
            'void main() {',
            '  vUv = uv;',
          ...
          const fragmentShader = [
            ...
            'varying vec2 vUv;',
            '',
            '// Generador pseudo-aleatorio exacto de Codrops',
            ...
            '  }',
            '',
            '  // Cuadrícula proporcional uniforme (bloques de píxel de ~12px exactos)',
            '  float cellSize = 12.0;',
            '  vec2 cells = max(vec2(1.0), floor(uResolution / cellSize));',
            '  vec2 gridUv = (floor(vUv * cells) + 0.5) / cells;',
            '',
            '  // Muestras de la textura: celda pixelada y texto continuo nítido',
            '  vec4 cellTex = texture2D(uTexture, gridUv);',
            '  vec4 origTex = texture2D(uTexture, vUv);',
            '',
            '  // Frente de onda exacto de Codrops: barre verticalmente de arriba a abajo',
            '  float height = 0.28;',
            '  float progress = (1.0 + height) - (uProgress * (1.0 + height + height));',
            '',
            '  float dist = 1.0 - distance(gridUv.y, progress);',
            '  float clampedDist = smoothstep(height, 0.0, distance(gridUv.y, progress));',
            '',
            '  float rand = random(gridUv);',
            '  float randDist = step(1.0 - height * rand, dist);',
            '  dist = step(1.0 - height, dist);',
            '',
            '  // Concentración de bloques de píxeles digitales ruidosos en el frente de onda',
            '  float blockAlpha = dist * (clampedDist + rand - 0.5 * (1.0 - randDist));',
            '  blockAlpha = clamp(blockAlpha, 0.0, 1.0);',
            '',
            '  // Detrás de la onda: se revela el texto',
            '  float isRevealed = step(progress, gridUv.y);',
            '',
            '  // Refinamiento suave: de bloques pixelados a texto tipográfico nítido',
            '  float refine = smoothstep(progress, progress + 0.16, gridUv.y);',
            '  vec4 currentText = mix(cellTex, origTex, refine);',
            '  currentText.rgba *= isRevealed;',
            '',
            '  // Detección de presencia tipográfica para no manchar márgenes transparentes',
            '  float textPresence = max(cellTex.a, origTex.a);',
            '  if (textPresence <= 0.01 && blockAlpha <= 0.01) {',
            '    discard;',
            '  }',
            '',
            '  // Bloque pixelado con color de acento industrial (#F1B541 / Solycal)',
            '  vec4 blockColor = vec4(uColor, blockAlpha * max(textPresence, 0.45));',
            '',
            '  // Fusión final idéntica a la demo de Codrops',
            '  vec4 finalColor = mix(currentText, blockColor, blockAlpha * 0.72);',
            '',
          ...
          // Colección de items gestionados
          const items = [];
          ...
          function setupItem(el) {
            ...
            // Disparador GSAP ScrollTrigger (start: 'top 85%')
            ScrollTrigger.create({
              trigger: el,
              start: 'top 85%',
              once: true,
              onEnter: () => triggerItemReveal(item)
            });
          }

          function triggerItemReveal(item) {
            if (item.triggered) return;
            item.triggered = true;

            const dpr = Math.min(window.devicePixelRatio || 1, 2);
            const renderData = renderHeadingToCanvas(item.el, dpr);
            if (!renderData) {
            ...
            const material = new THREE.ShaderMaterial({
              uniforms: {
                ...
                uColor: { value: new THREE.Color("#F1B541") }
              },
            ...
            mesh.position.set(rect.left + rect.width * 0.5, -(rect.top + rect.height * 0.5), 0);
            scene.add(mesh);
            ...
            // 5. ANIMACIÓN GSAP E INTERCAMBIO LIMPIO (0 a 1 en 1.35s estilo Codrops)
            gsap.to(material.uniforms.uProgress, {
              ...
              onComplete: () => {
                ...
                // Intercambio limpio: Pasar HTML original a opacity: 1 para accesibilidad y selección
                item.el.style.opacity = '1';

                // Liberación total de recursos GPU
          ...
          // Esperar a que las fuentes tipográficas del documento estén listas
          function startSetup() {
            targetElements.forEach(setupItem);
          }

          if (document.fonts && document.fonts.ready) {
            document.fonts.ready.then(startSetup);
          } else {
            startSetup();
          }

          // 6. LOOP DE RENDER RAF Y SEGUIMIENTO getBoundingClientRect()
          function renderRaf() {
            ...
            for (let i = 0; i < items.length; i++) {
              ...
              // Soporte para tarjetas o elementos con desplazamiento horizontal
              if (!item.triggered) {
                const rect = item.el.getBoundingClientRect();
                if (rect.left < winW * 0.85 && rect.right > 0 && rect.top < winH && rect.bottom > 0) {
                  triggerItemReveal(item);
                }
              }
              ...
              item.mesh.position.set(rect.left + rect.width * 0.5, -(rect.top + rect.height * 0.5), 0);
              item.material.uniforms.uResolution.value.set(rect.width, rect.height);
            }
        ...
        if (bitCanvas && bitPageContainer) {
          ...
          // Números en blanco sutil y tenue (sin amarillo para no interferir con el título)
          const color1 = 'rgba(255, 255, 255, 0.40)';
          const color2 = 'rgba(255, 255, 255, 0.22)';
          const dotColor1 = 'rgba(255, 255, 255, 0.30)';
          const dotColor2 = 'rgba(255, 255, 255, 0.15)';
...
<div id="services-page-container" class="relative bg-[#07080a] overflow-hidden">
  ...
  <div class="fixed inset-0 pointer-events-none z-0 overflow-hidden" id="bit-patterns-wrap" style="clip-path: inset(0 0 0 0);">
    <canvas id="bit-patterns-canvas" class="w-full h-full block opacity-30 hover:opacity-45 transition-opacity duration-700"></canvas>
  </div>

  <!-- Máscara de profundidad y contraste corporativo sobre el canvas para legibilidad óptima de textos y tarjetas -->
  <div class="fixed inset-0 bg-gradient-to-b from-[#07080a]/80 via-[#07080a]/40 to-[#07080a]/85 pointer-events-none z-[1]"></div>
  ...
  <div class="relative z-10">
    ...
    <section class="hscroll-section p-0 relative" id="services-hscroll">
      ...
      <div class="hscroll-track">
        ...
        <div class="hscroll-panel" data-panel-idx="0">
          <img src="assets/img/Servicios/caldereria-industrial.jpeg"
               alt="Calderería Industrial Solycal"
               class="hscroll-photo absolute inset-0 w-full h-full object-cover object-center pointer-events-none z-[1]"
               loading="eager">

          <!-- Desktop bilateral / lateral gradient -->
          <div class="hidden lg:block absolute inset-0 pointer-events-none z-[2]"
               style="background: linear-gradient(90deg, #07080a 0%, rgba(7,8,10,0.92) 24%, rgba(7,8,10,0.50) 42%, transparent 60%, transparent 85%, rgba(7,8,10,0.75) 100%);"></div>
          <div class="hidden lg:block absolute inset-0 pointer-events-none z-[2]"
               style="background: linear-gradient(180deg, rgba(7,8,10,0.6) 0%, transparent 15%, transparent 82%, rgba(7,8,10,0.8) 100%);"></div>

          <!-- Mobile bottom vignette: leaves upper 50-60% completely unobstructed for the photo -->
          <div class="lg:hidden absolute inset-0 pointer-events-none z-[2]"
               style="background: linear-gradient(180deg, rgba(7,8,10,0.15) 0%, transparent 25%, rgba(7,8,10,0.5) 45%, rgba(7,8,10,0.92) 70%, #07080a 95%);"></div>

          <div class="relative z-10 w-full max-w-7xl mx-auto px-5 sm:px-10 lg:px-16 h-full flex flex-col justify-end lg:justify-center pb-8 sm:pb-12 lg:pb-0">
            <div class="max-w-xl lg:max-w-lg xl:max-w-xl space-y-3 sm:space-y-4 lg:space-y-5">
              <div class="flex items-center gap-2 sm:gap-3">
                <span class="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-brand-yellow animate-pulse"></span>
                <span class="font-mono text-[10px] sm:text-xs text-brand-yellow uppercase tracking-widest">// 01 &bull; TRANSFORMACIÓN DE CHAPA</span>
                <span class="text-neutral-600 font-mono text-xs hidden sm:inline">|</span>
                <span class="font-mono text-[10px] sm:text-xs text-neutral-400 hidden sm:inline">DIVISIÓN PESADA</span>
              </div>
              <h2 class="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-display font-bold text-white tracking-tight leading-[1.12]">
                Calderería Industrial<br><span class="text-neutral-400 font-light">Pesada &amp; Ligera</span>
              </h2>
              <p class="text-xs sm:text-sm text-neutral-300 font-sans leading-relaxed max-w-lg">
              ...
              <div class="pt-2 sm:pt-3 border-t border-white/[0.08] space-y-1.5 sm:space-y-2">
                <span class="font-mono text-[10px] sm:text-xs text-neutral-400 uppercase tracking-widest block font-semibold">ESPECIFICACIONES INDUSTRIALES:</span>
                <ul class="font-mono text-[11px] sm:text-xs text-neutral-300 space-y-1 sm:space-y-1.5">
                  <li class="flex items-start gap-2.5">
                    <span class="w-1.5 h-1.5 bg-brand-yellow rounded-full shrink-0 mt-[4px]"></span>
              ...
              <div class="pt-1 sm:pt-2 flex items-center gap-2">
                <span class="font-mono text-[10px] sm:text-xs text-neutral-300 uppercase tracking-wider bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">Torrent &bull; Valencia &bull; Capacidad hasta 20 Tn</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="hscroll-panel" data-panel-idx="1">
          ...
          <div class="relative z-10 w-full max-w-7xl mx-auto px-5 sm:px-10 lg:px-16 h-full flex flex-col justify-end lg:justify-center pb-8 sm:pb-12 lg:pb-0">
            <div class="max-w-xl lg:max-w-lg xl:max-w-xl space-y-3 sm:space-y-4 lg:space-y-5">
              <div class="flex items-center gap-2 sm:gap-3">
                <span class="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-brand-yellow animate-pulse"></span>
                <span class="font-mono text-[10px] sm:text-xs text-brand-yellow uppercase tracking-widest">// 02 &bull; TECNOLOGÍA HYPERTHERM</span>
                <span class="text-neutral-600 font-mono text-xs hidden sm:inline">|</span>
                <span class="font-mono text-[10px] sm:text-xs text-neutral-400 hidden sm:inline">CORTE ALTA DEFINICIÓN</span>
              </div>
              <h2 class="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-display font-bold text-white tracking-tight leading-[1.12]">
              ...
              <div class="pt-1 sm:pt-2 flex items-center gap-2">
                <span class="font-mono text-[10px] sm:text-xs text-neutral-300 uppercase tracking-wider bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">Tolerancia &plusmn;0,3 mm &bull; Mesa 9.000 &times; 2.500 mm &bull; Hasta 50 mm</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="hscroll-panel" data-panel-idx="2">
          ...
          <div class="relative z-10 w-full max-w-7xl mx-auto px-5 sm:px-10 lg:px-16 h-full flex flex-col justify-end lg:justify-center pb-8 sm:pb-12 lg:pb-0">
            <div class="max-w-xl lg:max-w-lg xl:max-w-xl space-y-3 sm:space-y-4 lg:space-y-5">
              <div class="flex items-center gap-2 sm:gap-3">
                <span class="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-brand-yellow animate-pulse"></span>
                <span class="font-mono text-[10px] sm:text-xs text-brand-yellow uppercase tracking-widest">// 03 &bull; HOMOLOGACIONES OFICIALES</span>
                <span class="text-neutral-600 font-mono text-xs hidden sm:inline">|</span>
                <span class="font-mono text-[10px] sm:text-xs text-neutral-400 hidden sm:inline">CALIDAD CERTIFICADA</span>
              </div>
              <h2 class="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-display font-bold text-white tracking-tight leading-[1.12]">
              ...
              <div class="pt-1 sm:pt-2 flex items-center gap-2">
                <span class="font-mono text-[10px] sm:text-xs text-neutral-300 uppercase tracking-wider bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">EN ISO 15614 &bull; EN 15085-2 &bull; Homologación Oficial</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="hscroll-panel" data-panel-idx="3">
          ...
          <div class="relative z-10 w-full max-w-7xl mx-auto px-5 sm:px-10 lg:px-16 h-full flex flex-col justify-end lg:justify-center pb-8 sm:pb-12 lg:pb-0">
            <div class="max-w-xl lg:max-w-lg xl:max-w-xl space-y-3 sm:space-y-4 lg:space-y-5">
              <div class="flex items-center gap-2 sm:gap-3">
                <span class="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-brand-yellow animate-pulse"></span>
                <span class="font-mono text-[10px] sm:text-xs text-brand-yellow uppercase tracking-widest">// 04 &bull; MARCADO CE EN 1090-1</span>
                <span class="text-neutral-600 font-mono text-xs hidden sm:inline">|</span>
                <span class="font-mono text-[10px] sm:text-xs text-neutral-400 hidden sm:inline">CLASES EXC2 Y EXC3</span>
              </div>
              <h2 class="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-display font-bold text-white tracking-tight leading-[1.12]">
              ...
              <div class="pt-1 sm:pt-2 flex items-center gap-2">
                <span class="font-mono text-[10px] sm:text-xs text-neutral-300 uppercase tracking-wider bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">Marcado CE EN 1090-1 &bull; Clases EXC2 y EXC3</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="hscroll-panel" data-panel-idx="4">
          ...
          <div class="relative z-10 w-full max-w-7xl mx-auto px-5 sm:px-10 lg:px-16 h-full flex flex-col justify-end lg:justify-center pb-8 sm:pb-12 lg:pb-0">
            <div class="max-w-xl lg:max-w-lg xl:max-w-xl space-y-3 sm:space-y-4 lg:space-y-5">
              <div class="flex items-center gap-2 sm:gap-3">
                <span class="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-brand-yellow animate-pulse"></span>
                <span class="font-mono text-[10px] sm:text-xs text-brand-yellow uppercase tracking-widest">// 05 &bull; INSTALACIÓN EN PLANTA</span>
                <span class="text-neutral-600 font-mono text-xs hidden sm:inline">|</span>
                <span class="font-mono text-[10px] sm:text-xs text-neutral-400 hidden sm:inline">EJECUCIÓN DIRECTA</span>
              </div>
              <h2 class="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-display font-bold text-white tracking-tight leading-[1.12]">
              ...
              <div class="pt-1 sm:pt-2 flex items-center gap-2">
                <span class="font-mono text-[10px] sm:text-xs text-neutral-300 uppercase tracking-wider bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">Montaje en Obra Civil &bull; Cobertura Nacional</span>
              </div>
            </div>
          </div>
        </div>
...
<section id="contacto-hero" class="relative pt-24 sm:pt-32 pb-24 sm:pb-32 bg-[#07080a] overflow-hidden">
  ...
  <div class="w-full px-6 relative z-10 pointer-events-none [&_a]:pointer-events-auto mb-16 sm:mb-20 lg:mb-24">
    <div class="max-w-2xl lg:max-w-3xl space-y-6 reveal">
      ...
      <!-- CABECERA FRONTAL CON EFECTO PARTICLE TEXT (EPHYRION OPENPROCESSING #394108) -->
      <div id="particle-text-wrapper" class="relative w-full pointer-events-auto select-none min-h-[90px] sm:min-h-[140px] lg:min-h-[160px]">
        <h1 id="contact-hero-fallback" class="hero-title text-4xl sm:text-6xl lg:text-7xl font-display font-bold text-white tracking-tight leading-tight">
          Hablemos de tu próximo proyecto.
        </h1>
        <canvas id="particle-text-canvas" class="hidden block cursor-pointer transition-opacity duration-300" title="Haz clic o pasa el ratón para interactuar con las partículas"></canvas>
      </div>
...
<!-- SCRIPT: PARTICLE TEXT ENGINE (EPHYRION OPENPROCESSING #394108) -->
<script>
  (function() {
    const wrap = document.getElementById('particle-text-wrapper');
    ...
    class Particle {
      constructor(endX, endY, startX, startY, isAccent, w, h) {
        this.endX = endX;
        this.endY = endY;
        this.w = w;
        this.h = h;
        this.size = isAccent ? 2.2 : 1.6;
        this.color = isAccent ? '#F1B541' : (Math.random() < 0.2 ? '#FDE68A' : '#FFFFFF');
        this.duration = 0.8 + Math.random() * 1.5;
        this.ox = 0;
        this.oy = 0;
        this.vx = 0;
        this.vy = 0;
      ...
      reset(sx, sy) {
        ...
        const dist = 60 + Math.random() * spread * 0.9;
        this.anchorX = sx + Math.cos(angle) * dist;
        this.anchorY = sy + Math.sin(angle) * dist;
        this.currentX = sx;
        this.currentY = sy;
      }

      update(delta, mx, my, mInside) {
        if (this.f < 1) {
          this.f += delta / this.duration;
          if (this.f >= 1) this.f = 1;

          // Interpolación cuadrática suavizada de Ephyrion: 1 - (1 - f)^2
          const fi = 1 - Math.pow(1 - this.f, 2);

          // Curva Bézier cuadrática: start -> anchor -> end
          const x1 = this.startX + (this.anchorX - this.startX) * fi;
          const y1 = this.startY + (this.anchorY - this.startY) * fi;
          const x2 = this.anchorX + (this.endX - this.anchorX) * fi;
          const y2 = this.anchorY + (this.endY - this.anchorY) * fi;
          this.currentX = x1 + (x2 - x1) * fi;
          this.currentY = y1 + (y2 - y1) * fi;
        } else {
          this.currentX = this.endX;
          this.currentY = this.endY;
        }

        // Repulsión elástica interactiva al pasar el cursor
        if (mInside) {
          ...
          if (dSq < hoverRadius * hoverRadius && dSq > 0.1) {
            const d = Math.sqrt(dSq);
            const force = (1 - d / hoverRadius) * 14;
            this.vx += (dx / d) * force;
            this.vy += (dy / d) * force;
          }
        }

        // Muelle elástico de retorno a la letra
        this.vx += -this.ox * 0.15;
        this.vy += -this.oy * 0.15;
        this.vx *= 0.82;
        this.vy *= 0.82;
        this.ox += this.vx;
        this.oy += this.vy;
      ...
      draw(c) {
        ...
        c.fillRect(this.currentX + this.ox, this.currentY + this.oy, this.size, this.size);
      }
    }
    ...
    function build() {
      ...
      let fontSize = 68;
      if (containerWidth < 380) fontSize = 34;
      else if (containerWidth < 480) fontSize = 40;
      else if (containerWidth < 768) fontSize = 52;
      else if (containerWidth < 1024) fontSize = 62;
      ...
      const words = ["Hablemos", "de", "tu", "próximo", "proyecto."];
      const lines = [];
      ...
      for (let i = 1; i < words.length; i++) {
        ...
        if (offCtx.measureText(test).width < maxTextWidth) {
          currentLine = test;
        } else {
      ...
      offCtx.fillRect(0, 0, containerWidth, totalHeight);
      offCtx.font = font;
      ...
      for (let py = 0; py < off.height; py += step) {
        for (let px = 0; px < off.width; px += step) {
          ...
          if (brightness > 85) {
            ...
            newParticles.push(new Particle(targetX, targetY, centerX, centerY, isAccent, containerWidth, totalHeight));
          }
        }
      }
      ...
      fallback.classList.remove('hero-title');
    ...
    function render(now) {
      ...
      const delta = Math.min((now - lastTime) * 0.001, 0.06);
      lastTime = now;

      const w = parseFloat(canvas.style.width) || canvas.width;
      const h = parseFloat(canvas.style.height) || canvas.height;

      // Efecto estelas de luz (Trails de Ephyrion)
      ctx.fillStyle = 'rgba(7, 8, 10, 0.24)';
      ctx.fillRect(0, 0, w, h);

      for (let i = 0; i < particles.length; i++) {
        particles[i].update(delta, mouseX, mouseY, mouseInside);
    ...
    // Clic o toque para dispersar y reiniciar la llegada hacia el cursor
