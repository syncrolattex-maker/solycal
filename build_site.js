      ...
      if (typeof gsap !== 'undefined') {
        ...
        (function initWebGLPixelReveal() {
          ...
          const targetElements = Array.from(document.querySelectorAll('h1:not(.hero-title), h2')).filter(el => {
            return !el.closest('#joby-nav-overlay') && !el.closest('#solycal-loader') && !el.closest('#contacto-hero') && !el.closest('#particle-text-anchor') && el.textContent.trim().length > 0;
          });
...
<section id="contacto-hero" class="relative pt-24 sm:pt-32 pb-24 sm:pb-32 bg-[#07080a] overflow-hidden">
  ...
  <div class="w-full px-6 relative z-10 pointer-events-none [&_a]:pointer-events-auto mb-16 sm:mb-20 lg:mb-24">
    <div class="max-w-2xl lg:max-w-3xl">
      ...
      <!-- VERSIÓN ESCRITORIO (>= md): Contenedor dedicado para efecto Particle Text (en escritorio NO se muestra título normal, solo partículas) -->
      <div id="particle-text-anchor" class="hidden md:block relative w-full min-h-[170px] lg:min-h-[200px] mb-8 select-none cursor-pointer" title="Haz clic o pasa el ratón para interactuar con las partículas a ancho completo">
        <h1 id="contact-hero-fallback" class="sr-only">
          Hablemos de tu próximo proyecto.
        </h1>
      </div>
...
<script>
  (function() {
    ...
    class Particle {
      constructor(endX, endY, isAccent) {
        this.endX = endX;
        this.endY = endY;
      ...
      reset(sx, sy) {
        this.f = 0;
        this.duration = 0.85 + Math.random() * 1.55;
        ...
        const spread = Math.max(canvasW || window.innerWidth, canvasH || 700);
        const dist = 120 + Math.random() * spread * 0.75;
        this.anchorX = this.startX + Math.cos(angle) * dist;
        this.anchorY = this.startY + Math.sin(angle) * dist;
      ...
      update(delta, mx, my, mInside) {
        ...
        if (mInside) {
          ...
          if (dSq < hoverRadius * hoverRadius && dSq > 0.1) {
            ...
            const force = (1 - d / hoverRadius) * 16;
            this.vx += (dx / d) * force;
            this.vy += (dy / d) * force;
          }
        }

        // Muelle elástico de retorno a su posición en la letra
        this.vx += -this.ox * 0.14;
        this.vy += -this.oy * 0.14;
        this.vx *= 0.82;
        this.vy *= 0.82;
        this.ox += this.vx;
        this.oy += this.vy;
    ...
    function build() {
      ...
      const fontSize = canvasW < 1024 ? 50 : 66;
      ...
      // Word wrapping del texto: "Hablemos de tu próximo proyecto."
      ...
      off.height = Math.round(offTextHeight * dpr);
      offCtx.setTransform(dpr, 0, 0, dpr, 0, 0);

      offCtx.fillStyle = '#000000';
      offCtx.fillRect(0, 0, offTextWidth, offTextHeight);
      offCtx.font = font;
      ...
      for (let py = 0; py < off.height; py += step) {
        for (let px = 0; px < off.width; px += step) {
          ...
          if (brightness > 85) {
            const targetX = textTargetX + px / dpr;
            const targetY = textTargetY + py / dpr;
            const isAccent = (Math.random() < 0.12);
            newParticles.push(new Particle(targetX, targetY, isAccent));
          }
        }
      }
    ...
    function render(now) {
      ...
      // Estelas alfa transparentes para que el fondo se vea con naturalidad a pantalla completa (Trails de Ephyrion)
